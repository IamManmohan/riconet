...
public class ConsignmentAppointmentServiceImpl implements ConsignmentAppointmentService {
  ...
  public void processAppointmentNotification(AppointmentNotificationDTO dto) {
    ...
    switch (dto.getNotificationType()) {
      ...
      case APPOINTMENT_MISSED:
        notificationList = processAppointmentMissed(new DateTime(dto.getLastExecutionTime()), now,
            statusList);
        notificationList.forEach(notification ->
            sendNotifications(notification, ZoomPropertyName.APPOINTMENT_MISSED_EMAIL,
      ...
      case APPOINTMENT_MISSED_SUMMARY:
        ...
        notificationMap = notificationList.stream()
            .collect(Collectors
                .groupingBy(notification -> notification.getResponsibleLocation().getId()));
        notificationMap.keySet().forEach(list ->
            sendNotificationList(notificationMap.get(list),
      ...
      case APPOINTMENT_NOT_OFD_FIRST_HALF:
        processAppointmentNotOfd(statusList, now,
  ...
  private void processAppointmentNotOfd(List<ConsignmentStatus> statusList, DateTime start,
      DateTime end) {
    statusList.add(ConsignmentStatus.OUT_FOR_DELIVERY);
    List<AppointmentNotification> notificationList = processAppointmentMissed(start, end,
        statusList);
    Map<Long, List<AppointmentNotification>> notificationMap = notificationList.stream()
        .collect(
            Collectors.groupingBy(notification -> notification.getResponsibleLocation().getId()));
    notificationMap.keySet().forEach(list ->
  ...
  private List<AppointmentNotification> processAppointmentMissed(final DateTime start,
      final DateTime end, List<ConsignmentStatus> statusList) {
    ...
    List<Consignment> consignments = consignmentService
        .findByIdInAndStatusNotInAndDeliveryHandoverIsNull(consignmentIdList,
            statusList);
    ...
    return consignments.stream()
        .map(consignment ->
            processAppointmentMissedConsignment(consignment,
                cnToScheduleMap.get(consignment.getId()), locationMap, defaultCcList, bccList)
        ).collect(Collectors.toList());
  ...
  private ConsignmentSchedule getCurrentSchedule(
    ...
    Optional<ConsignmentSchedule> present = consignmentScheduleList.stream()
        .filter(consignmentSchedule ->
  ...
  private void processLateDelivery(AppointmentNotificationDTO dto) {
    ...
    appointmentNotification
        .setCnote(consignmentService.getCnoteByIdAndIsActive(cnHistory.getConsignmentId()));
    ...
    Set<String> defaultCcList = emailService.getEmails(EmailDlName.APPOINTMENT_NOTIFICATION_CC);
    updateStakeHolders(appointmentNotification, loc, defaultCcList, bccList);
    DateTime deliveryTime = (new DateTime(dto.getDeliveryTime()))
        .withZone(DateTimeZone.forID(IST_TIME_ZONE_ID));
    DateTime appointmentTime = (new DateTime(dto.getAppoitnmentTime()))
        .withZone(DateTimeZone.forID(IST_TIME_ZONE_ID));
    if (Days.daysBetween(deliveryTime.toLocalDate(), appointmentTime.toLocalDate()).getDays()
        == 0) {
      sendNotifications(appointmentNotification,
  ...
  private void processFakeUndeliveryReason(AppointmentNotificationDTO dto) {
    ...
    appointmentNotification.setResponsibleLocation(getLocationDto(loc));
    appointmentNotification
        .setCnote(consignmentService.getCnoteByIdAndIsActive(dto.getConsignmentId()));
  ...
  private void sendNotifications(AppointmentNotification notification,
      ZoomPropertyName emailPropertyName, ZoomPropertyName subjectPropertyName) {
    String templateString = zoomPropertyService.getString(emailPropertyName);
    Boolean isEmailEnabled = zoomPropertyService
        .getBoolean(ZoomPropertyName.APPOINTMENT_NOTIFICATION_ENABLED, false);
    String subjectTemplate = zoomPropertyService
        .getString(subjectPropertyName);//get from zoom property
    if (templateString != null && isEmailEnabled && subjectTemplate != null) {
      ...
      emailService.sendAppointmentEmail(notification.getEmailIdList(), notification.getCcList(),
          notification.getBccList(), subject, body, null);
    }
  }


  private void sendNotificationList(List<AppointmentNotification> notificationList,
      ZoomPropertyName emailPropertyName, ZoomPropertyName subjectPropertyName) {
    ...
    if (templateString != null && isEmailEnabled) {
      ...
      try(SXSSFWorkbook wb = new SXSSFWorkbook(100)) {
        ...
        emailService.sendAppointmentEmail(notificationList.get(0).getEmailIdList(),
            notificationList.get(0).getCcList(), notificationList.get(0).getBccList(), subject,
            body, file);
      } catch (IOException e) {
  ...
  private String designEmailTemplate(AppointmentNotification appointmentNotification,
      String template) {
    Map<String, String> valuesMap = new HashMap<>();
    valuesMap.put("cnote", appointmentNotification.getCnote());
    valuesMap.put("user", appointmentNotification.getResponsiblePerson() == null ? "-"
        : appointmentNotification.getResponsiblePerson().getName());
    valuesMap.put("locationCode", appointmentNotification.getResponsibleLocation() == null ? "-"
  ...
  private Set<String> getCcList(Location loc) {
    ...
    ccList.addAll(zoomUserMasterService.getActiveZoomUsersByLocationInAndZoomUserType(locIds,
        ZoomUserType.ZOOM_CLM.name(), ZoomUserType.ZOOM_TECH_SUPPORT.name()).stream()
        .map(ZoomUser::getEmail).collect(Collectors.toList()));
    ccList.addAll(zoomUserMasterService.getActiveZoomUsersByLocationInAndZoomUserType(locIds,
        ZoomUserType.ZOOM_RM.name(), ZoomUserType.ZOOM_TECH_SUPPORT.name()).stream()
        .map(ZoomUser::getEmail).collect(Collectors.toList()));
    ccList.addAll(zoomUserMasterService.getActiveZoomUsersByLocationAndZoomUserType(pc.getId(),
        ZoomUserType.ZOOM_PCM.name(), ZoomUserType.ZOOM_TECH_SUPPORT.name()).stream()
        .map(ZoomUser::getEmail).collect(Collectors.toList()));
    return ccList;
  }
}
