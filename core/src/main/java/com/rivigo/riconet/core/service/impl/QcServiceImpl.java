...
public class QcServiceImpl implements QcService {
    ...
    ticketList.forEach(
        ticketDTO -> {
          if (ZoomTicketingConstant.QC_RECHECK_TYPE_ID.equals(ticketDTO.getTypeId())
            ...
            ticketingService.closeTicket(ticketDTO, ZoomTicketingConstant.QC_AUTO_CLOSURE_MESSAGE_DISPATCH);
          }
        });
  }
  ...
  public void consumeUnloadingEvent(ConsignmentBasicDTO unloadingData) {
    ...
    ticketList.forEach(
        ticketDTO ->
           ticketingService.closeTicket(ticketDTO, ZoomTicketingConstant.QC_AUTO_CLOSURE_MESSAGE_NO_QC_GROUP));
  ...
  private void createTicketsIfNeeded(
    ...
    if (reCheckQcNeeded) {
      ...
      if (autoClose) {
        ticketingService.closeTicket(dto, ZoomTicketingConstant.QC_AUTO_CLOSURE_MESSAGE_NO_QC_GROUP);
      }
    }
