...
public class EmailServiceImpl implements EmailService {
  ...
  @Override
  public void sendShortageEmail(Collection<String> toRecipients, Collection<String> ccRecipients,
      Collection<String> bccRecipients, String subject, String body, File file) {
    MailUtils.sendEmail(SHORTAGE_EMAIL_ID, SHORTAGE_EMAIL_PASS, new ArrayList<>(toRecipients),
        new ArrayList<>(ccRecipients), new ArrayList<>(bccRecipients), subject, body, file);
  }

  @Override
  public Set<String> getEmails(EmailDlName dl) {
    return emailDLRepository.findByDlAndIsActive(dl, 1).stream().map(EmailDL::getEmail)
        .collect(Collectors.toSet());
  }

  @Override
  public void sendDocumentIssueEmail(Collection<String> toRecipients,
      Collection<String> ccRecipients, Collection<String> bccRecipients, String subject,
      String body, File file) {
    MailUtils.sendEmail(DOCUMENT_EMAIL_ID, DOCUMENT_EMAIL_PASS, new ArrayList<>(toRecipients),
        new ArrayList<>(ccRecipients), new ArrayList<>(bccRecipients), subject, body, file);
  }

  @Override
  public void sendAppointmentEmail(Collection<String> toRecipients, Collection<String> ccRecipients,
      Collection<String> bccRecipients, String subject, String body, File file) {
    MailUtils.sendEmail(APPOINTMENT_EMAIL_ID, APPOINTMENT_EMAIL_PASS, new ArrayList<>(toRecipients),
        new ArrayList<>(ccRecipients), new ArrayList<>(bccRecipients), subject, body, file);
  }
