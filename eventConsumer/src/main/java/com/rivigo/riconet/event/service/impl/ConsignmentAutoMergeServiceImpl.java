...
public class ConsignmentAutoMergeServiceImpl implements ConsignmentAutoMergeService {
    ...
    public void autoMergeSecondaryConsignment(NotificationDTO notificationDTO) {
        ...
        String location = notificationDTO.getMetadata().get(ZoomCommunicationFieldNames.CURRENT_LOCATION_ID.name());
            ...
            try {
                JsonNode responseJson =
                        apiClientService.getEntity(
                                Collections.singletonList(cnote), HttpMethod.POST, "/deps/auto-merge", null, zoomBackendBaseUrl);
                log.debug("response {}", responseJson);
            } catch (IOException e) {
                log.error("Exception occurred while merging cn in zoom backend {}", ExceptionUtils.getFullStackTrace(e));
            }
