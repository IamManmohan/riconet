...
public class ZoomCommunicationsConsumer extends ConsumerModel {
  ...
  @Autowired private ZoomCommunicationEmailService zoomCommunicationEmailService;
  ...
  public void processMessage(String str) throws IOException {
    ...
    ZoomCommunicationsDTO zoomCommunicationsDTO = null;
    try {
      zoomCommunicationsDTO = objectMapper.readValue(str, ZoomCommunicationsDTO.class);
    } catch (Exception e) {
      log.error("failed", e);
    }
    zoomCommunicationsService.processNotification(zoomCommunicationsDTO);
    zoomCommunicationEmailService.processNotification(zoomCommunicationsDTO);
  }
}
