...
WORKDIR /code/
COPY --from=builder /src/notificationConsumer/build/distributions/notificationConsumer.zip /code/
...
ARG PROPERTIES_PATH=src/main/resources
RUN echo $PROPERTIES_PATH
COPY --from=builder $PROPERTIES_PATH /etc/zoom/notification/application.properties
ARG SPRING_PROFILE=staging
RUN echo $SPRING_PROFILE
ARG XMX_XMS_VALUE=512m
RUN echo $XMX_XMS_VALUE
ENV JAVA_OPTS="-Xms$XMX_XMS_VALUE -Xmx$XMX_XMS_VALUE -Dspring.config.location=/etc/zoom/notification -Dlogin.profiles.active=$LOGIN_PROFILE"
RUN echo $JAVA_OPTS
WORKDIR /code/notificationConsumer/bin
RUN sed -i 's%DEFAULT_JVM_OPTS=""%DEFAULT_JVM_OPTS="-Xms1024m -Xmx1024m -Dspring.config.location=/etc/zoom/notification -Dlogin.profiles.active=$SPRING_PROFILE"%g' notificationConsumer
